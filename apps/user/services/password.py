...
class PasswordService:
    ...
    def send_reset_password_link(cls, email: str) -> None:
        try:
            ...
            reset_password_link = (
                f"https://{domain_name}/reset-password/{reset_password_signature}"
            )
            context = {
                "user_notification_salutation": user.get_full_name,
    ...
    def reset_password(cls, signature: str, new_password: str) -> None:
        ...
        try:
            user_pk = signer.unsign(
                signature, max_age=settings.BOYCOTT_RESET_PASSWORD_EXPIRATION_DELTA
            )
